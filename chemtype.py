...
GROUND_TRUTH_DICT = {}
f = open('D:\CV_project\stanford_ocsr\chemtype2\ocr_groundtruth.txt') 
...
CORNER_TRUTH_DICT = {}
g = open('D:\CV_project\stanford_ocsr\chemtype2\corners_groundtruth.txt')
...
def classify_bonds(edge_file,img,classifier,label_dict,template_dict_file,rect_w=6,empty_thresh=0.3):
  ...
  for i,line_segment in enumerate(edges):
    ...
    if color == 'none':
      continue
    pt1 = line_segment[0]  # This might be a tuple of np.float32
    pt2 = line_segment[1]  # Same here

    # Ensure the points are tuples of integers
    pt1_int = (int(pt1[0]), int(pt1[1]))  # Convert to integer
    pt2_int = (int(pt2[0]), int(pt2[1]))  # Convert to integer

    # Draw the line
    cv2.line(color_im, pt1_int, pt2_int, color, thickness=5)
  for key in template_dict.keys():
    if len(template_dict[key]) != 0:
      color = COLOR_DICT_OCR[key]
      for box in template_dict[key]:
...
for path in PATHS:
  ...
  for image in os.listdir(path):
    if image[len(image)-4:len(image)] != '.png':
      continue
    try:
      corr, n_corr = classify_bonds('D:/CV_project/stanford_ocsr/chemtype2/pickles/' + image[0:11] + '_edges.pickle', path+image, classifier_svm, label_dict_svm, 'D:/CV_project/stanford_ocsr/chemtype2/pickles/' + image[0:11] + '_tol_eq_0.77_template_bb.pickle')
      print("step1")
      corr_mol += corr
      corr_edge += n_corr
      total += 1
    ...
    try:
      ...
      corr, n_corr = classify_bonds('D:/CV_project/stanford_ocsr/chemtype2/pickles/' + image[0:10] + '__edges.pickle', path+image, classifier_svm, label_dict_svm, 'D:/CV_project/stanford_ocsr/chemtype2/pickles/' + image[0:10] + '_tol_eq_0.77_template_bb.pickle')
      print("step2")
      corr_mol += corr
      corr_edge += n_corr
      total += 1
